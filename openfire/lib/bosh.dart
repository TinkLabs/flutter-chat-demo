...
class BoshClient extends ServiceType {
  ...
  BoshClient(ChatConnection connection) {
...
/* * Variable: strip
  ...
  StanzaBuilder _buildBody() {
    StanzaBuilder bodyWrap = ChatUtil.$build(
        'body', {'rid': this.rid++, 'xmlns': ChatUtil.NS['HTTPBIND']});
    if (this.sid != null) {
  ...
  _connect([int wait, int hold, String route]) {
    ...
    BoshRequest req =
        new BoshRequest(body.tree(), null, body.tree().getAttribute("rid"));
  ...
  _attach(String jid, String sid, int rid, Function callback, int wait,
    ...
    this._conn.domain = ChatUtil.getDomainFromJid(this._conn.jid);
    ...
    this._conn.changeConnectStatus(ChatUtil.Status['ATTACHED'], null);
  }
  ...
  _restore(String jid, Function callback, int wait, int hold, int wind) {
    ...
    if (session != null &&
        ...
        session.jid &&
        (jid == null ||
            ChatUtil.getBareJidFromJid(session.jid) ==
                ChatUtil.getBareJidFromJid(jid) ||
            // If authcid is null, then it's an anonymous login, so
            // we compare only the domains:
            ((ChatUtil.getNodeFromJid(jid) == null) &&
                (ChatUtil.getDomainFromJid(session.jid) == jid)))) {
  ...
  _connectCb(xml.XmlElement bodyWrap) {
    ...
    if (typ != null && typ == "terminate") {
      ...
      ChatUtil.error("BOSH-Connection failed: " + cond);
      conflict = bodyWrap.findAllElements("conflict");
      if (cond != null) {
        if (cond == "remote-stream-error" && conflict.length > 0) {
          cond = "conflict";
        }
        this._conn.changeConnectStatus(ChatUtil.Status['CONNFAIL'], cond);
      } else {
        this._conn.changeConnectStatus(ChatUtil.Status['CONNFAIL'], "unknown");
      }
      this._conn.doDisconnect(cond);
      return ChatUtil.Status['CONNFAIL'];
    }
  ...
  /* * PrivateFunction: _callProtocolErrorHandlers
     ...
     */
  _callProtocolErrorHandlers(BoshRequest req) {
  ...
  void _hitError(int reqStatus) {
    this.errors++;
    ChatUtil.warn("request errored, status: " +
        reqStatus.toString() +
        ", number of errors: " +
  ...
  _onIdle() {
    ...
    if (this._conn.authenticated &&
      ...
      ChatUtil.info("no requests during idle cycle, sending " + "blank request");
      data.add(null);
    }
    ...
    if (this._requests.length < 2 && data.length > 0) {
      ...
      this._conn.data = [];
      BoshRequest req = new BoshRequest(
    ...
    if (this._requests.length > 0) {
      ...
      if (this._requests[0].dead != null) {
        if (this._requests[0].timeDead() >
            (ChatUtil.SECONDARY_TIMEOUT * this.wait).floor()) {
          this._throttledRequestHandler();
        }
      }

      if (time_elapsed > (ChatUtil.TIMEOUT * this.wait).floor()) {
        ChatUtil.warn("Request " +
            this._requests[0].id.toString() +
            " timed out, over " +
            (ChatUtil.TIMEOUT * this.wait).floor().toString() +
  ...
  /* * PrivateFunction: _getRequestStatus
     ...
     */
  int _getRequestStatus(BoshRequest req, [num def]) {
    int reqStatus;
    if (req.response != null) {
      try {
        reqStatus = req.response.statusCode;
      } catch (e) {
        ChatUtil.error("Caught an error while retrieving a request's status, " +
            "reqStatus: " +
            reqStatus.toString());
      }
    }
  ...
  /* * PrivateFunction: _onRequestStateChange
     ...
     *    (Request) req - The request that is changing readyState.
     */
  _onRequestStateChange(Function func, BoshRequest req) {
    ChatUtil.debug("request id " +
        req.id.toString() +
        "." +
        req.sends.toString() +
        " state changed to " +
    ...
    if (reqStatus == 200) {
      ...
      if (reqStatus >= 400 && reqStatus < 500) {
        this._conn.changeConnectStatus(ChatUtil.Status['DISCONNECTING'], null);
    ...
    if (!valid_request && !too_many_retries) {
      ...
      this._conn.changeConnectStatus(ChatUtil.Status['CONNFAIL'], "giving-up");
    }
  }
  ...
  _processRequest(int i) {
    BoshRequest req = this._requests[i];
    ...
    var primaryTimeout = (time_elapsed is num &&
        time_elapsed > (ChatUtil.TIMEOUT * this.wait).floor());
    var secondaryTimeout = (req.dead != null &&
        req.timeDead() > (ChatUtil.SECONDARY_TIMEOUT * this.wait).floor());
    ...
    if (primaryTimeout || secondaryTimeout || requestCompletedWithServerError) {
      ...
      req.xhr.close();
      this._requests[i] =
          new BoshRequest(req.xmlData, req.origFunc, req.rid, req.sends);
    ...
    if (req.response == null) {
      ...
      if (req.sends > 1) {
        ...
        num backoff =
            min((ChatUtil.TIMEOUT * this.wait).floor(), pow(req.sends, 3)) *
                1000;
      ...
      ChatUtil.debug("_processRequest: " +
          (i == 0 ? "first" : "second") +
  ...
  _sendFunc(BoshRequest req) {
    ...
    try {
      ...
      ChatUtil.error("XHR open failed: " + e2.toString());
      if (!this._conn.connected) {
        this
            ._conn
            .changeConnectStatus(ChatUtil.Status['CONNFAIL'], "bad-service");
      }
  ...
  /* * PrivateFunction: _removeRequest
     ...
     */
  void _removeRequest(BoshRequest req) {
    ChatUtil.debug("removing request");
  ...
  xml.XmlElement reqToData(dynamic req) {
    req = req as BoshRequest;
    return this._reqToData(req);
  }

  xml.XmlElement _reqToData(BoshRequest req) {
    try {
  ...
  /* * PrivateFunction: _sendTerminate
     ...
     */
  _sendTerminate(pres) {
    ChatUtil.info("_sendTerminate was called");
  ...
  _throttledRequestHandler() {
    if (this._requests == null) {
      ...
      ChatUtil.debug("_throttledRequestHandler called with " +
...
class BoshRequest {
  ...
  BoshRequest(xml.XmlElement elem, Function func, String rid, [int sends]) {
    this.id = ++ChatUtil.requestId;
    this.xmlData = elem;
    this.data = ChatUtil.serialize(elem);
...
/* * PrivateFunction: getResponse
  ...
  xml.XmlElement getResponse() {
    ...
    try {
      ...
      ChatUtil.error("responseXML: " + ChatUtil.serialize(node));
      if (node == null) {
        throw {'message': 'Parsing produced null node'};
      }
    } catch (e) {
      // if (node.name == "parsererror") {
      ChatUtil.error("invalid response received" + e.toString());
      ChatUtil.error("responseText: " + body);
      throw "parsererror";
      //}
    }
    return node;
