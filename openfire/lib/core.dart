...
import 'package:openfire/websocket.dart';
...
class ChatUtil {
  ...
  static Map<String, dynamic> XHTML = {
    ...
    /* * Function: XHTML.validTag
         ...
         */
    'validTag': (String tag) {
      for (int i = 0; i < ChatUtil.XHTML['tags'].length; i++) {
        if (tag == ChatUtil.XHTML['tags'][i]) {
          return true;
        }
      }
      return false;
    ...
    'validAttribute': (String tag, String attribute) {
      if (ChatUtil.XHTML['attributes'][tag] != null &&
          ChatUtil.XHTML['attributes'][tag].length > 0) {
        for (int i = 0; i < ChatUtil.XHTML['attributes'][tag].length; i++) {
          if (attribute == ChatUtil.XHTML['attributes'][tag][i]) {
            return true;
          }
        }
      }
    ...
    'validCSS': (style) {
      for (int i = 0; i < ChatUtil.XHTML['css'].length; i++) {
        if (style == ChatUtil.XHTML['css'][i]) {
          return true;
        }
      }
      return false;
...
/* * Function: xmlGenerator
  ...
  static xml.XmlBuilder xmlGenerator() {
    ...
    ChatUtil._xmlGenerator = ChatUtil._makeGenerator();
    //}
    return ChatUtil._xmlGenerator;
  }
  ...
  static xml.XmlNode xmlElement(String name, {dynamic attrs, String text}) {
    ...
    xml.XmlBuilder builder = ChatUtil.xmlGenerator();
...
/* * Function: createHtml
  ...
  static xml.XmlNode createHtml(xml.XmlNode elem) {
    ...
    if (elem.nodeType == xml.XmlNodeType.ELEMENT) {
      ...
      //tag = elem.
      if (ChatUtil.XHTML['validTag'](tag)) {
        try {
          el = ChatUtil.copyElement(elem);
        } catch (e) {
          // invalid elements
          el = ChatUtil.xmlTextNode('');
        }
      } else {
        el = ChatUtil.copyElement(elem);
      }
      ...
      el = ChatUtil.xmlTextNode(elem.toString());
    }
    return el;
  }
...
/* * Function: unescapeNode
  ...
  static String getDomainFromJid(String jid) {
    String bare = ChatUtil.getBareJidFromJid(jid);
    if (bare.indexOf("@") < 0) {
      return bare;
    } else {
...
/* * PrivateFunction: _handleError
  ...
  static handleError(Error e) {
    if (e.stackTrace != null) {
      ChatUtil.fatal(e.stackTrace.toString());
    }
    if (e.toString() != null) {
      ChatUtil.fatal("error: " +
          e.hashCode.toString() +
          " - " +
  ...
  static log(int level, String msg) {
    if (level != ChatUtil.LogLevel['FATAL']) {
      print(msg);
    }
  }
  ...
  static debug(String msg) {
    ChatUtil.log(ChatUtil.LogLevel['DEBUG'], msg);
  }
  ...
  static info(String msg) {
    ChatUtil.log(ChatUtil.LogLevel['INFO'], msg);
  }
  ...
  static warn(String msg) {
    ChatUtil.log(ChatUtil.LogLevel['WARN'], msg);
  }
  ...
  static error(String msg) {
    ChatUtil.log(ChatUtil.LogLevel['ERROR'], msg);
  }
  ...
  static fatal(String msg) {
    ChatUtil.log(ChatUtil.LogLevel['FATAL'], msg);
  }
...
/* * Function: serialize
  ...
  static void addConnectionPlugin(String name, ptype) {
    ChatUtil._connectionPlugins[name] = ptype;
  }
  ...
  static StanzaHandler handler(Function handler, String ns, String name,
    ...
    if (stanzaHandler.options['matchBareFromJid'] != null &&
      ...
      stanzaHandler.from = (from != null && from.isNotEmpty)
          ? ChatUtil.getBareJidFromJid(from)
          : null;
    } else {
 ...
 */
  ...
  static StanzaBuilder $build(String name, Map<String, dynamic> attrs) {
    return ChatUtil.builder(name, attrs: attrs);
  }

  static StanzaBuilder $msg([Map<String, dynamic> attrs]) {
    return ChatUtil.builder("message", attrs: attrs);
  }

  static StanzaBuilder $iq([Map<String, dynamic> attrs]) {
    return ChatUtil.builder("iq", attrs: attrs);
  }

  static StanzaBuilder $pres([Map<String, dynamic> attrs]) {
    return ChatUtil.builder("presence", attrs: attrs);
  }
 ...
 */
  static ChatConnection connection(String service, [Map options]) {
    ChatConnection connection = new ChatConnection(service, options);
    return connection;
  }

  static ChatWebSocket websocket(ChatConnection connection) {
    return new ChatWebSocket(connection);
  }

  static BoshClient bosh(ChatConnection connection) {
    return new BoshClient(connection);
  }
