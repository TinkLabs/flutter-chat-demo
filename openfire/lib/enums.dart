...
import 'package:openfire/bosh.dart';
import 'package:openfire/core.dart';
import 'package:openfire/md5.dart';
import 'package:openfire/plugins/administration.dart';
import 'package:openfire/plugins/bookmark.dart';
import 'package:openfire/plugins/caps.dart';
import 'package:openfire/plugins/chat-notifications.dart';
import 'package:openfire/plugins/disco.dart';
import 'package:openfire/plugins/last-activity.dart';
import 'package:openfire/plugins/muc.dart';
import 'package:openfire/plugins/pep.dart';
import 'package:openfire/plugins/plugins.dart';
import 'package:openfire/plugins/privacy.dart';
import 'package:openfire/plugins/private-storage.dart';
import 'package:openfire/plugins/pubsub.dart';
import 'package:openfire/plugins/register.dart';
import 'package:openfire/plugins/roster.dart';
import 'package:openfire/plugins/vcard-temp.dart';
import 'package:openfire/sessionstorage.dart';
import 'package:openfire/sha1.dart';
import 'package:openfire/utils.dart';
...
class StanzaBuilder {
  ...
  StanzaBuilder(String name, [Map<String, dynamic> attrs]) {
    ...
    if (name == "presence" || name == "message" || name == "iq") {
      if (attrs != null && attrs['xmlns'] == null) {
        attrs['xmlns'] = ChatUtil.NS['CLIENT'];
      } else if (attrs == null) {
        attrs = {'xmlns': ChatUtil.NS['CLIENT']};
      }
    }
    ...
    this.nodeTree = ChatUtil.xmlElement(name, attrs: attrs);
  ...
  /* * Function: toString
     ...
     */
  String toString() {
    return ChatUtil.serialize(this.nodeTree);
  }
  ...
  StanzaBuilder c(String name, [Map<String, dynamic> attrs, dynamic text]) {
    xml.XmlNode child = ChatUtil.xmlElement(name, attrs: attrs, text: text);
    ...
    currentNode.children.add(ChatUtil.copyElement(xmlElement));
  ...
  StanzaBuilder cnode(xml.XmlNode elem) {
    xml.XmlNode newElem = ChatUtil.copyElement(elem);
    xml.XmlNode currentNode = this.nodeTree.children[0];
    for (int i = 1; i < this.node.length; i++) {
      currentNode = currentNode.children[this.node[i]];
    }
    if (newElem != null) currentNode.children.add(ChatUtil.copyElement(newElem));
    this.node.add(currentNode.children.length - 1);
    return this;
  }
  ...
  StanzaBuilder t(String text) {
    ...
    currentNode.children.add(ChatUtil.copyElement(xml.XmlText(text ?? '')));
    return this;
  }
  ...
  StanzaBuilder h(String html) {
    xml.XmlNode fragment = ChatUtil.xmlElement('body');

    // force the browser to try and fix any invalid HTML tags
    fragment.children.add(ChatUtil.xmlTextNode(html));

    // copy cleaned html into an xml dom
    xml.XmlNode xhtml = ChatUtil.createHtml(fragment);
    xml.XmlNode currentNode = this.nodeTree.children[0];
    for (int i = 1; i < this.node.length; i++) {
      currentNode = currentNode.children[this.node[i]];
    }
    currentNode.children.add(ChatUtil.copyElement(xhtml));
    return this;
  }
}
...
/* * PrivateFunction: getNamespace
  ...
  bool namespaceMatch(xml.XmlNode elem) {
    ...
    if (this.ns == null || this.ns.isEmpty) {
      ...
      ChatUtil.forEachChild(elem, null, (child) {
  ...
  bool isMatch(xml.XmlNode node) {
    ...
    if (this.options['matchBareFromJid']) {
      from = ChatUtil.getBareJidFromJid(from);
    }
    bool withId = false;
    ...
    if (this.namespaceMatch(elem) &&
        (this.name == null || ChatUtil.isTagEqual(elem, this.name)) &&
...
class ChatConnection {
  ...
  RegisterPlugin get register {
    return ChatUtil.connectionPlugins['register'];
  }

  DiscoPlugin get disco {
    return ChatUtil.connectionPlugins['disco'];
  }

  RosterPlugin get roster {
    return ChatUtil.connectionPlugins['roster'];
  }

  AdministrationPlugin get admin {
    return ChatUtil.connectionPlugins['admin'];
  }

  CapsPlugin get caps {
    return ChatUtil.connectionPlugins['caps'];
  }

  MucPlugin get muc {
    return ChatUtil.connectionPlugins['muc'];
  }

  BookMarkPlugin get bookmarks {
    return ChatUtil.connectionPlugins['bookmarks'];
  }

  LastActivity get lastactivity {
    return ChatUtil.connectionPlugins['lastactivity'];
  }

  PepPlugin get pep {
    return ChatUtil.connectionPlugins['pep'];
  }

  PrivacyPlugin get privacy {
    return ChatUtil.connectionPlugins['privacy'];
  }

  PubsubPlugin get pubsub {
    return ChatUtil.connectionPlugins['pubsub'];
  }

  PrivateStorage get private {
    return ChatUtil.connectionPlugins['private'];
  }

  VCardTemp get vcard {
    return ChatUtil.connectionPlugins['vcard'];
  }

  ChatStatesNotificationPlugin get chatstates {
    return ChatUtil.connectionPlugins['chatstates'];
  }
  ...
  ChatConnection(String service, [Map options]) {
    ...
    if (service.indexOf("ws:") == 0 ||
        ...
        proto.indexOf("ws") == 0) {
      this._proto = ChatUtil.websocket(this);
    } else {
      this._proto = ChatUtil.bosh(this);
    }
    /* The connected JID. */
    this.jid = "";
    /* the JIDs domain */
    ...
    // initialize plugins
    ChatUtil.connectionPlugins.forEach((String key, PluginClass value) {
      ChatUtil.connectionPlugins[key].init(this);
    });
  }
  initializeFunction() {
    this._reset = () {
    ...
    this._connectCb = (req, Function _callback, String raw) {
      ...
      try {
        ...
        this._changeConnectStatus(
            ChatUtil.Status['CONNFAIL'], ChatUtil.ErrorCondition['BAD_FORMAT']);
        this._doDisconnect(ChatUtil.ErrorCondition['BAD_FORMAT']);
      }
      if (bodyWrap == null) {
        return;
      }
      ...
      if (raw != null) {
        ...
        this.rawInput(ChatUtil.serialize(bodyWrap));
      }
      //}

      int conncheck = this._proto.connectCb(bodyWrap);
      if (conncheck == ChatUtil.Status['CONNFAIL']) {
        return;
      }

      // Check for the stream:features tag
      bool hasFeatures;
      if (bodyWrap.getAttribute('xmlns') == ChatUtil.NS['STREAM']) {
      ...
      if (mechanisms.length > 0) {
        for (int i = 0; i < mechanisms.length; i++) {
          mech = ChatUtil.getText(mechanisms.elementAt(i));
  ...
  addConnectionPlugin(String name, PluginClass ptype) {
    ChatUtil.addConnectionPlugin(name, ptype);
  }
  ...
  _connect(String jid, String pass, ConnectCallBack callback,
     ...
     */
    this.authzid = ChatUtil.getBareJidFromJid(this.jid);

    /* * Variable: authcid
         *  Authentication identity (User name).
         */
    this.authcid = authcid ?? ChatUtil.getNodeFromJid(this.jid);
    ...
    this.domain = ChatUtil.getDomainFromJid(this.jid);

    this._changeConnectStatus(ChatUtil.Status['CONNECTING'], null);
  ...
  StanzaTimedHandler addTimedHandler(int period, Function handler) {
    StanzaTimedHandler thand = ChatUtil.timedHandler(period, handler);
  ...
  StanzaHandler addHandler(Function handler, String ns, String name,
    ...
    StanzaHandler hand =
        ChatUtil.handler(handler, ns, name, type, id, from, options);
  ...
  registerSASLMechanisms(mechanisms) {
    ...
    mechanisms = mechanisms ??
        [
          ChatUtil.SASLAnonymous,
          ChatUtil.SASLExternal,
          ChatUtil.SASLMD5,
          ChatUtil.SASLOAuthBearer,
          ChatUtil.SASLXOAuth2,
          ChatUtil.SASLPlain,
          ChatUtil.SASLSHA1
        ];
  ...
  disconnect([String reason = ""]) {
    this._changeConnectStatus(ChatUtil.Status['DISCONNECTING'], reason);

    ChatUtil.info("Disconnect was called because: " + reason);
    if (this.connected) {
      StanzaBuilder pres;
      this.disconnecting = true;
      if (this.authenticated) {
        pres = ChatUtil.$pres(
            {'xmlns': ChatUtil.NS['CLIENT'], 'type': 'unavailable'});
      }
  ...
  _changeConnectStatus(int status, [String condition, xml.XmlNode elem]) {
    ...
    ChatUtil.connectionPlugins.forEach((String key, PluginClass plugin) {
    ...
    if (this.connectCallback != null) {
      try {
        ...
        if (e is Error) ChatUtil.handleError(e);
        ChatUtil.error("User connection callback caused an " +
            "exception: " +
            e.toString());
      }
    }
  ...
  _doDisconnect([condition]) {
    ...
    this._changeConnectStatus(ChatUtil.Status['DISCONNECTED'], condition);
    this.connected = false;
  }
  ...
  _dataRecv(req, [String raw]) {
    ...
    if (raw != null) {
      ...
      this.rawInput(ChatUtil.serialize(elem));
    }
    //}
    ...
    if (type != null && type == "terminate") {
      ...
      if (cond != null) {
        ...
        this._changeConnectStatus(ChatUtil.Status['CONNFAIL'], cond);
      } else {
        this._changeConnectStatus(ChatUtil.Status['CONNFAIL'],
            ChatUtil.ErrorCondition['UNKOWN_REASON']);
      }
    ...
    ChatUtil.forEachChild(elem, null, (child) {
      ...
      for (int i = 0; i < newList.length; i++) {
        ...
        try {
          ...
          ChatUtil.warn('Removing Strophe handlers due to uncaught exception: ' +
              e.toString());
        }
      }
    });
  ...
  _noAuthReceived([Function _callback]) {
    ...
    this._changeConnectStatus(
        ChatUtil.Status['CONNFAIL'], ChatUtil.ErrorCondition['NO_AUTH_MECH']);
    if (_callback != null) {
      _callback();
    }
    this._doDisconnect();
...
/* set connectCb(){
  ...
  bool _attemptSASLAuth(List<StropheSASLMechanism> mechanisms) {
    ...
    for (int i = 0; i < mechanisms.length; ++i) {
      ...
      StanzaBuilder requestAuthExchange = ChatUtil.$build("auth",
          {'xmlns': ChatUtil.NS['SASL'], 'mechanism': this._saslMechanism.name});
  ...
  /* * PrivateFunction: _attemptLegacyAuth
    ...
    */
  _attemptLegacyAuth() {
    if (ChatUtil.getNodeFromJid(this.jid) == null) {
      // we don't have a node, which is required for non-anonymous
      // client connections
      this._changeConnectStatus(ChatUtil.Status['CONNFAIL'],
          ChatUtil.ErrorCondition['MISSING_JID_NODE']);
      this.disconnect(ChatUtil.ErrorCondition['MISSING_JID_NODE']);
    } else {
      // Fall back to legacy authentication
      this._changeConnectStatus(ChatUtil.Status['AUTHENTICATING'], null);
      this._addSysHandler(this._auth1Cb, null, null, null, "_auth_1");
      this.send(ChatUtil.$iq({'type': "get", 'to': this.domain, 'id': "_auth_1"})
          .c("query", {'xmlns': ChatUtil.NS['AUTH']})
          .c("username", {})
          .t(ChatUtil.getNodeFromJid(this.jid))
          .tree());
    }
  }
  ...
  bool _saslChallengeCb(elem) {
    String challenge =
        String.fromCharCodes(base64.decode(ChatUtil.getText(elem)));
    String response = this._saslMechanism.onChallenge(this, challenge);
    StanzaBuilder stanza =
        ChatUtil.$build('response', {'xmlns': ChatUtil.NS['SASL']});
    if (response != "") {
  ...
  /* * PrivateFunction: _auth1Cb authenticate            
  ...
  _auth1Cb(elem) {
    ...
    StanzaBuilder iq = ChatUtil.$iq({'type': "set", 'id': "_auth_2"})
        .c('query', {'xmlns': ChatUtil.NS['AUTH']})
        .c('username', {})
        .t(ChatUtil.getNodeFromJid(this.jid))
        .up()
    ...
    if (ChatUtil.getResourceFromJid(this.jid) == null ||
        ChatUtil.getResourceFromJid(this.jid).isEmpty) {
      // since the user has not supplied a resource, we pick
      // a default one here.  unlike other auth methods, the server
      // cannot do this for us.
      this.jid = ChatUtil.getBareJidFromJid(this.jid) + '/strophe';
    }
    iq.up().c('resource', {}).t(ChatUtil.getResourceFromJid(this.jid));
  ...
  bool _saslSuccessCb(elem) {
    ...
    ChatUtil.info("SASL authentication succeeded.");
    ...
    streamFeatureHandlers.add(this._addSysHandler((elem) {
      return this.wrapper(streamFeatureHandlers, elem);
    }, ChatUtil.NS['STREAM'], "features", null, null));
  ...
  bool _saslAuth1Cb(element) {
    ...
    if (!this.doBind) {
      this._changeConnectStatus(ChatUtil.Status['AUTHFAIL'], null);
      return false;
    } else {
      this._addSysHandler(this._saslBindCb, null, null, null, "_bind_auth_2");

      String resource = ChatUtil.getResourceFromJid(this.jid);
      if (resource != null && resource.isNotEmpty) {
        this.send(ChatUtil.$iq({'type': "set", 'id': "_bind_auth_2"})
            .c('bind', {"xmlns": ChatUtil.NS['BIND']})
            .c('resource', {})
            .t(resource)
            .tree());
      } else {
        this.send(ChatUtil.$iq({'type': "set", 'id': "_bind_auth_2"})
            .c('bind', {'xmlns': ChatUtil.NS['BIND']}).tree());
      }
    }
    return false;
  }
  ...
  bool _saslBindCb(xml.XmlElement elem) {
    if (elem.getAttribute("type") == "error") {
      ChatUtil.info("SASL binding failed.");
      List<xml.XmlElement> conflict = elem.findAllElements("conflict");
      String condition;
      if (conflict.length > 0) {
        condition = ChatUtil.ErrorCondition['CONFLICT'];
      }
      this._changeConnectStatus(ChatUtil.Status['AUTHFAIL'], condition, elem);
      return false;
    }
    ...
    if (bind.length > 0) {
      ...
      if (jidNode.length > 0) {
        this.jid = ChatUtil.getText(jidNode[0]);

        if (this.doSession) {
          this._addSysHandler(
              this._saslSessionCb, null, null, null, "_session_auth_2");
          this.send(ChatUtil.$iq({'type': "set", 'id': "_session_auth_2"})
              .c('session', {'xmlns': ChatUtil.NS['SESSION']}).tree());
        } else {
          this.authenticated = true;
          this._changeConnectStatus(ChatUtil.Status['CONNECTED'], null);
        }
      }
      return false;
    } else {
      ChatUtil.info("SASL binding failed.");
      this._changeConnectStatus(ChatUtil.Status['AUTHFAIL'], null, elem);
      return false;
    }
  }
  ...
  bool _saslSessionCb(xml.XmlElement elem) {
    if (elem.getAttribute("type") == "result") {
      ...
      ChatUtil.info("Session creation failed.");
      this._changeConnectStatus(ChatUtil.Status['AUTHFAIL'], null, elem);
      return false;
    }
    return false;
  ...
  StanzaHandler _addSysHandler(
      Function handler, String ns, String name, String type, String id) {
    StanzaHandler hand = ChatUtil.handler(handler, ns, name, type, id);
    hand.user = false;
    this.addHandlers.add(hand);
    return hand;
  }
  ...
  bool _onDisconnectTimeout() {
    this._changeConnectStatus(ChatUtil.Status['CONNTIMEOUT'], null);
...
class StropheSASLMechanism {
  ...
  bool test(ChatConnection connection) {
    return true;
  }
  /* jshint unused:true */
  ...
  /* * PrivateFunction: onStart
    ...
    */
  void onStart(ChatConnection connection) {
  ...
  /* * PrivateFunction: onChallenge
    ...
    */
  /* jshint unused:false */
  String onChallenge(ChatConnection connection,
...
class StropheSASLSHA1 extends StropheSASLMechanism {
  ...
  String _onChallenge(ChatConnection connection, String challenge) {
