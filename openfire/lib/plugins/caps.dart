...
class CapsPlugin extends PluginClass {
  ...
  init(ChatConnection c) {
    this.connection = c;
    ChatUtil.addNamespace('CAPS', "http://jabber.org/protocol/caps");
    if (this.connection.disco == null) {
      throw {'error': "disco plugin required!"};
    }
    this.connection.disco.addFeature(ChatUtil.NS['CAPS']);
    this.connection.disco.addFeature(ChatUtil.NS['DISCO_INFO']);
  ...
  sendPres() {
    ...
    return this.connection.send(ChatUtil.$pres().cnode(caps.tree()));
  }
  ...
  StanzaBuilder createCapsNode() {
    ...
    return ChatUtil.$build("c", {
      'xmlns': ChatUtil.NS['CAPS'],
      'hash': this._hash,
      'node': node,
