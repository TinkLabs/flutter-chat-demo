...
class DiscoPlugin extends PluginClass {
  ...
  init(ChatConnection conn) {
    ...
    // disco info
    conn.addHandler(
        this._onDiscoInfo, ChatUtil.NS['DISCO_INFO'], 'iq', 'get', null, null);
    // disco items
    conn.addHandler(
        this._onDiscoItems, ChatUtil.NS['DISCO_ITEMS'], 'iq', 'get', null, null);
  }
  ...
  info(String jid,
    ...
    Map<String, String> attrs = {'xmlns': ChatUtil.NS['DISCO_INFO']};
  ...
  StanzaBuilder _buildIQResult(
    ...
    StanzaBuilder iqresult = ChatUtil.$iq({'type': 'result', id: id});
  ...
  bool _onDiscoItems(XmlElement stanza) {
    Map<String, String> queryAttrs = {'xmlns': ChatUtil.NS['DISCO_ITEMS']};
    String node =
        stanza.findAllElements('query').toList()[0].getAttribute('node');
    List items;
    if (node != null && node.isNotEmpty) {
