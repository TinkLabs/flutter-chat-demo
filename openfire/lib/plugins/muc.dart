 ...
 */
  ...
  init(ChatConnection conn) {
    ...
    ChatUtil.addNamespace('MUC_OWNER', ChatUtil.NS['MUC'] + "#owner");
    ChatUtil.addNamespace('MUC_ADMIN', ChatUtil.NS['MUC'] + "#admin");
    ChatUtil.addNamespace('MUC_USER', ChatUtil.NS['MUC'] + "#user");
    ChatUtil.addNamespace('MUC_ROOMCONF', ChatUtil.NS['MUC'] + "#roomconfig");
    return ChatUtil.addNamespace('MUC_REGISTER', "jabber:iq:register");
  }
  ...
  join(String room, String nick,
    ...
    pres = ChatUtil.$pres({'from': this.connection.jid, 'to': roomNick}).c(
        "x", {'xmlns': ChatUtil.NS['MUC']});
    if (historyAttrs != null) {
      pres = pres.c("history", historyAttrs).up();
    }
    if (password != null && password.isNotEmpty) {
      pres.cnode(ChatUtil.xmlElement("password", attrs: [], text: password));
    }
        ...
        if (stanza.name.qualified == "message") {
          ...
          if (xquery.length > 0) {
            ...
            for (int i = 0, len = xquery.length; i < len; i++) {
              ...
              if (xmlns != null &&
                  new RegExp(xmlns).hasMatch(ChatUtil.NS['MUC'])) {
  ...
  leave(String room, String nick, [Function handlerCb, String exitMsg]) {
    ...
    StanzaBuilder presence = ChatUtil.$pres({
      'type': "unavailable",
      'id': presenceid,
  ...
  String message(String room, String nick, String message,
    ...
    msg.up();
    if (htmlMessage != null) {
      msg.c("html", {'xmlns': ChatUtil.NS['XHTML_IM']}).c(
          "body", {'xmlns': ChatUtil.NS['XHTML']}).h(htmlMessage);
  ...
  String invite(String room, String receiver, [String reason]) {
    ...
    StanzaBuilder invitation = ChatUtil
        .$msg({'from': this.connection.jid, 'to': room, 'id': msgid}).c('x',
            {'xmlns': ChatUtil.NS['MUC_USER']}).c('invite', {'to': receiver});
    if (reason != null) {
  ...
  queryOccupants(String room, [Function successCb, Function errorCb]) {
    Map<String, String> attrs = {'xmlns': ChatUtil.NS['DISCO_ITEMS']};
    StanzaBuilder info = ChatUtil
  ...
  configure(String room, [Function successCb, Function errorCb]) {
    StanzaBuilder config = ChatUtil.$iq({'to': room, 'type': "get"}).c(
        "query", {'xmlns': ChatUtil.NS['MUC_OWNER']});
  ...
  cancelConfigure(String room) {
    StanzaBuilder config = ChatUtil.$iq({'to': room, 'type': "set"}).c("query", {
      'xmlns': ChatUtil.NS['MUC_OWNER']
  ...
  saveConfiguration(String room, List<XmlElement> config,
    ...
    StanzaBuilder iq = ChatUtil.$iq({'to': room, 'type': "set"}).c(
        "query", {'xmlns': ChatUtil.NS['MUC_OWNER']});
  ...
  createInstantRoom(String room, [Function successCb, Function errorCb]) {
    StanzaBuilder roomiq = ChatUtil.$iq({'to': room, 'type': "set"}).c("query", {
      'xmlns': ChatUtil.NS['MUC_OWNER']
  ...
  _modifyPrivilege(String room, StanzaBuilder item,
    ...
    StanzaBuilder iq = ChatUtil.$iq({'to': room, 'type': "set"}).c(
        "query", {'xmlns': ChatUtil.NS['MUC_ADMIN']}).cnode(item.currentNode);
  ...
  modifyRole(String room, String nick, String role,
      [String reason, Function handlerCb, Function errorCb]) {
    StanzaBuilder item = ChatUtil.$build("item", {'nick': nick, 'role': role});
  ...
  modifyAffiliation(String room, String jid, String affiliation,
      [String reason, Function handlerCb, Function errorCb]) {
    StanzaBuilder item =
        ChatUtil.$build("item", {'jid': jid, 'affiliation': affiliation});
    return this._modifyPrivilege(room, item, reason, handlerCb, errorCb);
  }
  ...
  setStatus(String room, String user, [String show, String status]) {
    ...
    StanzaBuilder presence =
        ChatUtil.$pres({'from': this.connection.jid, 'to': roomNick});
    if (show != null) {
  ...
  registrationRequest(String room, Function handleCb, [Function errorCb]) {
    StanzaBuilder iq = ChatUtil
        .$iq({'to': room, 'from': this.connection.jid, 'type': "get"}).c(
            "query", {'xmlns': ChatUtil.NS['MUC_REGISTER']});
  ...
  listRooms(String server, [Function handleCb, errorCb]) {
    StanzaBuilder iq = ChatUtil
        .$iq({'to': server, 'from': this.connection.jid, 'type': "get"}).c(
            "query", {'xmlns': ChatUtil.NS['DISCO_ITEMS']});
    return this.connection.sendIQ(iq.tree(), handleCb, errorCb);
  }

  String testAppendNick(String room, String nick) {
    String domain, node;
    node = ChatUtil.escapeNode(ChatUtil.getNodeFromJid(room));
    domain = ChatUtil.getDomainFromJid(room);
...
class XmppRoom {
  ...
  XmppRoom(MucPlugin client, String name, String nick1, String password1) {
    ...
    this.name = ChatUtil.getBareJidFromJid(this.name);
  ...
  Map<String, dynamic> _parsePresence(XmlElement pres) {
    ...
    data['nick'] = ChatUtil.getResourceFromJid(pres.getAttribute("from"));
    ...
    for (int i = 0, len = ref.length; i < len; i++) {
      ...
      switch (c.name.qualified) {
        ...
        case "show":
          ...
          break;
        case "x":
          if (c.getAttribute("xmlns") == ChatUtil.NS['MUC_USER']) {
            ref2 = c.children;
