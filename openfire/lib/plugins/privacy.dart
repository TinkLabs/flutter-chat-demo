...
class PrivacyPlugin extends PluginClass {
  ...
  getListNames(
    ...
    this.connection.sendIQ(
        ...
        }).c("query", {'xmlns': ChatUtil.NS['PRIVACY']}).tree(),
        (XmlNode element) {
      XmlElement stanza;
  ...
  deleteList(String name, Function successCallback, Function failCallback) {
    ...
    this.connection.sendIQ(
        ChatUtil.$iq({
          'type': "set",
          'id': this.connection.getUniqueId("privacy")
        }).c("query", {'xmlns': ChatUtil.NS['PRIVACY']}).c(
            "list", {'name': name}).tree(), () {
      ...
      if (successCallback != null)
        try {
          ...
          ChatUtil.error("Exception while running callback after removing list");
        }
    }, failCallback);
  }
    ...
    this.connection.sendIQ(listIQ.tree(), () {
      ...
      if (successCallback != null)
        try {
          ...
          ChatUtil.error("Exception in callback when saving list " + name);
        }
    }, failCallback);
    return true;
  }
    ...
    this.connection.sendIQ(
      ...
      if (successCallback != null)
        try {
          ...
          ChatUtil.error("Exception while running callback after loading list");
        }
    }, failCallback);
  }
  ...
  setActive(String name, [Function successCallback, Function failCallback]) {
    StanzaBuilder iq = ChatUtil
        .$iq({'type': "set", 'id': this.connection.getUniqueId("privacy")}).c(
            "query", {'xmlns': ChatUtil.NS['PRIVACY']}).c("active");
  ...
  setDefault(String name, [Function successCallback, Function failCallback]) {
    StanzaBuilder iq = ChatUtil
        .$iq({'type': "set", 'id': this.connection.getUniqueId("privacy")}).c(
            "query", {'xmlns': ChatUtil.NS['PRIVACY']}).c("default");
    if (name != null && name.isNotEmpty) iq.attrs({'name': name});
    this.connection.sendIQ(iq.tree(), () {
      this._default = name;
      if (successCallback != null)
