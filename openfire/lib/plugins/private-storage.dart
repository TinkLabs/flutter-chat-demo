...
class PrivateStorage extends PluginClass {
  ...
  set(String tag, String ns, data, [Function success, Function error]) {
    ...
    ns = ns ?? 'namespace';
    tag = tag ?? 'tag';
    StanzaBuilder iq = ChatUtil.$iq({'type': 'set', 'id': id}).c(
        'query', {'xmlns': ChatUtil.NS['PRIVATE']}).c(tag, {'xmlns': ns});
  ...
  get(String tag, String ns, Function success, [Function error]) {
    ...
    ns = ns ?? 'namespace';
    tag = tag ?? 'tag';
    StanzaBuilder iq = ChatUtil.$iq({'type': 'get', 'id': id}).c(
        'query', {'xmlns': ChatUtil.NS['PRIVATE']}).c(tag, {'xmlns': ns});
