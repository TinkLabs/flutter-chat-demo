 ...
 */
  form(String ns, Map<String, dynamic> options) {
    XmlNode xmlElement = ChatUtil.xmlElement('x',
        attrs: {"xmlns": "jabber:x:data", "type": "submit"});
    PubsubBuilder aX = this.cnode(xmlElement);
    aX
        .cnode(ChatUtil.xmlElement('field',
            attrs: {"var": "FORM_TYPE", "type": "hidden"}))
        .cnode(ChatUtil.xmlElement('value'))
        .t(ns)
        .up()
        .up();
    options.forEach((String key, value) {
      aX
          .cnode(ChatUtil.xmlElement('field', attrs: {"var": key}))
          .cnode(ChatUtil.xmlElement('value'))
...
class PubsubPlugin extends PluginClass {
  PubsubPlugin() {
    ...
    statusChanged = (status, condition) {
      if (this._autoService && status == ChatUtil.Status['CONNECTED']) {
        this.service =
            'pubsub.' + ChatUtil.getDomainFromJid(this.connection.jid);
...
*/
  ...
  init(ChatConnection conn) {
    ...
    ChatUtil.addNamespace('PUBSUB_SUBSCRIBE_OPTIONS',
        ChatUtil.NS['PUBSUB'] + "#subscribe_options");
    ChatUtil.addNamespace('PUBSUB_ERRORS', ChatUtil.NS['PUBSUB'] + "#errors");
    ChatUtil.addNamespace('PUBSUB_EVENT', ChatUtil.NS['PUBSUB'] + "#event");
    ChatUtil.addNamespace('PUBSUB_OWNER', ChatUtil.NS['PUBSUB'] + "#owner");
    ChatUtil.addNamespace(
        'PUBSUB_AUTO_CREATE', ChatUtil.NS['PUBSUB'] + "#auto-create");
    ChatUtil.addNamespace(
        'PUBSUB_PUBLISH_OPTIONS', ChatUtil.NS['PUBSUB'] + "#publish-options");
    ChatUtil.addNamespace(
        'PUBSUB_NODE_CONFIG', ChatUtil.NS['PUBSUB'] + "#node_config");
    ChatUtil.addNamespace('PUBSUB_CREATE_AND_CONFIGURE',
        ChatUtil.NS['PUBSUB'] + "#create-and-configure");
    ChatUtil.addNamespace('PUBSUB_SUBSCRIBE_AUTHORIZATION',
        ChatUtil.NS['PUBSUB'] + "#subscribe_authorization");
    ChatUtil.addNamespace(
        'PUBSUB_GET_PENDING', ChatUtil.NS['PUBSUB'] + "#get-pending");
    ChatUtil.addNamespace('PUBSUB_MANAGE_SUBSCRIPTIONS',
        ChatUtil.NS['PUBSUB'] + "#manage-subscriptions");
    ChatUtil.addNamespace(
        'PUBSUB_META_DATA', ChatUtil.NS['PUBSUB'] + "#meta-data");
    ChatUtil.addNamespace('ATOM', "http://www.w3.org/2005/Atom");

    if (conn.disco != null) conn.disco.addFeature(ChatUtil.NS['PUBSUB']);
  }
  ...
  createNode(String node,
    ...
    PubsubBuilder iq = new PubsubBuilder('iq', {
      'from': ChatUtil.getBareJidFromJid(this.jid),
      'to': service,
      'type': 'set',
      'id': iqid
    ...
    if (options != null) {
      ...
      iq.form(ChatUtil.NS['PUBSUB_NODE_CONFIG'], options);
    }
    if (callback != null)
  ...
  deleteNode(String node, [Function callback]) {
    ...
    StanzaBuilder iq = ChatUtil.$iq(
            {'from': this.jid, 'to': this.service, 'type': 'set', 'id': iqid})
        .c('pubsub', {'xmlns': ChatUtil.NS['PUBSUB_OWNER']}).c(
            'delete', {'node': node});
  ...
  discoverNodes(
    ...
    service = service != null && service.isNotEmpty ? service : this.service;
    StanzaBuilder iq =
        ChatUtil.$iq({'from': this.jid, 'to': service, 'type': 'get'})
            .c('query', {'xmlns': ChatUtil.NS['DISCO_ITEMS']});
  ...
  getConfig(String node, Function callback) {
    ...
    StanzaBuilder iq = ChatUtil.$iq(
            {'from': this.jid, 'to': this.service, 'type': 'get', 'id': iqid})
        .c('pubsub', {'xmlns': ChatUtil.NS['PUBSUB_OWNER']}).c(
            'configure', {'node': node});
  ...
  String getDefaultNodeConfig(Function callback) {
    ...
    StanzaBuilder iq = ChatUtil.$iq(
            {'from': this.jid, 'to': this.service, 'type': 'get', 'id': iqid})
        .c('pubsub', {'xmlns': ChatUtil.NS['PUBSUB_OWNER']}).c('default');
  ...
  subscribe(String node,
    ...
    String jid = this.jid;
    if (barejid) jid = ChatUtil.getBareJidFromJid(jid);
    service = service != null && service.isNotEmpty ? service : this.service;
    ...
    if (options != null) {
      ...
      c.form(ChatUtil.NS['PUBSUB_SUBSCRIBE_OPTIONS'], options);
    }
  ...
  unsubscribe(String node, String jid,
    ...
    service = service != null && service.isNotEmpty ? service : this.service;
    StanzaBuilder iq = ChatUtil.$iq(
            {'from': this.jid, 'to': service, 'type': 'set', 'id': iqid})
        .c('pubsub', {'xmlns': ChatUtil.NS['PUBSUB']}).c('unsubscribe', {
      'node': node,
      'jid': jid ?? ChatUtil.getBareJidFromJid(connection.jid)
    });
  ...
  String items(String node, [Function success, Function error, int timeout]) {
    //ask for all items
    StanzaBuilder iq =
        ChatUtil.$iq({'from': this.jid, 'to': this.service, 'type': 'get'})
            .c('pubsub', {'xmlns': ChatUtil.NS['PUBSUB']}).c(
                'items', {'node': node});
  ...
  getSubscriptions(Function callback) {
    ...
    StanzaBuilder iq = ChatUtil.$iq(
            {'from': this.jid, 'to': this.service, 'type': 'get', 'id': iqid})
        .c('pubsub', {'xmlns': ChatUtil.NS['PUBSUB']}).c('subscriptions');
  ...
  getAffiliations(String node, Function callback) {
    ...
    Map<String, String> attrs = {}, xmlns = {'xmlns': ChatUtil.NS['PUBSUB']};
    if (node != null && node.isNotEmpty) {
      attrs['node'] = node;
      xmlns = {'xmlns': ChatUtil.NS['PUBSUB_OWNER']};
    }
  ...
  setAffiliation(
    ...
    StanzaBuilder iq = ChatUtil.$iq(
            {'from': this.jid, 'to': this.service, 'type': 'set', 'id': iqid})
        .c('pubsub', {'xmlns': ChatUtil.NS['PUBSUB_OWNER']}).c('affiliations', {
      'node': node
  ...
  publishAtom(String node, List atoms, Function callback) {
    ...
    for (int i = 0; i < atoms.length; i++) {
      ...
      PubsubBuilder data =
          ChatUtil.$build("entry", {'xmlns': ChatUtil.NS['ATOM']});
      entries.add({
