...
class RegisterPlugin extends PluginClass {
  ...
  init(ChatConnection conn) {
    ...
    int i = 0;
    ChatUtil.Status.forEach((String key, int value) {
      i = max(i, ChatUtil.Status[key]);
    });
    ...
    /* extend name space
         ...
         *              from XEP 77.
         */
    ChatUtil.addNamespace('REGISTER', 'jabber:iq:register');
    ChatUtil.Status['REGIFAIL'] = i + 1;
    ChatUtil.Status['REGISTER'] = i + 2;
    ChatUtil.Status['REGISTERED'] = i + 3;
    ChatUtil.Status['CONFLICT'] = i + 4;
    ChatUtil.Status['NOTACCEPTABLE'] = i + 5;
    if (conn.disco != null) {
      if (conn.disco.addFeature is Function)
        conn.disco.addFeature(ChatUtil.NS['REGISTER']);
    ...
    conn.authenticate = (List<StropheSASLMechanism> matched) {
      if (matched == null) {
        ...
        if (this.fields['username'] == null ||
          ...
          ChatUtil.info("Register a JID first!");
          return;
        }
        ...
        conn.jid = jid;
        conn.authzid = ChatUtil.getBareJidFromJid(conn.jid);
        conn.authcid = ChatUtil.getNodeFromJid(conn.jid);
  ...
  connect(String domain, ConnectCallBack callback,
    ...
    ChatConnection conn = this.connection;
    this.domain = ChatUtil.getDomainFromJid(domain);
    this.instructions = "";
    this.fields = {};
    this.registered = false;
  ...
  _register_cb(req, Function _callback, String raw) {
    ChatConnection conn = this.connection;
    ChatUtil.info("_register_cb was called");
    conn.connected = true;
    ...
    if (raw != null) {
      ...
      conn.rawInput(ChatUtil.serialize(bodyWrap));
    }
    //}
    ...
    if (register.length == 0) {
      conn.changeConnectStatus(ChatUtil.Status['REGIFAIL'], null);
      return true;
    }
    ...
    conn.sendIQ(ChatUtil.$iq({'type': "get"}).c(
        "query", {'xmlns': ChatUtil.NS['REGISTER']}).tree());
  ...
  _get_register_cb(dynamic elem) {
    ...
    if (queries.length != 1) {
      conn.changeConnectStatus(ChatUtil.Status['REGIFAIL'], "unknown");
      return false;
    }
    ...
    for (int i = 0; i < query.children.length; i++) {
      ...
      if (field.name.qualified.toLowerCase() == 'instructions') {
        ...
        conn.register.instructions = ChatUtil.getText(field);
        continue;
      ...
      conn.register.fields[field.name.qualified.toLowerCase()] =
          ChatUtil.getText(field);
    }
    conn.changeConnectStatus(ChatUtil.Status['REGISTER'], null);
    return false;
  }
  ...
  submit() {
    ...
    List<String> fields;
    ChatConnection conn = this.connection;
    StanzaBuilder query = ChatUtil
        .$iq({'type': "set"}).c("query", {'xmlns': ChatUtil.NS['REGISTER']});
    // set required fields
  ...
  _submit_cb(XmlElement stanza) {
    ...
    if (queries.length > 0) {
      ...
      for (int i = 0; i < query.children.length; i++) {
        ...
        if (field.name.qualified.toLowerCase() == 'instructions') {
          // this is a special element
          // it provides info about given data fields in a textual way
          this.instructions = ChatUtil.getText(field);
          continue;
        }
    ...
    if (stanza.getAttribute("type") == "error") {
      ...
      ChatUtil.info("Registration failed.");
      ...
      if (error == 'conflict') {
        conn.changeConnectStatus(ChatUtil.Status['CONFLICT'], error);
      } else if (error == 'not-acceptable') {
        conn.changeConnectStatus(ChatUtil.Status['NOTACCEPTABLE'], error);
      } else {
        String text =
            ChatUtil.getText(errors[0].findElements('text').toList()[0]) +
                '/$error';
        conn.changeConnectStatus(ChatUtil.Status['REGIFAIL'], text ?? error);
      }
    } else {
      ChatUtil.info("Registration successful.");

      conn.changeConnectStatus(ChatUtil.Status['REGISTERED'], null);
    }
