...
class RosterPlugin extends PluginClass {
  ...
  init(ChatConnection conn) {
    ...
    Function newCallback = (int status, condition, ele) {
      if (status == ChatUtil.Status['ATTACHED'] ||
          status == ChatUtil.Status['CONNECTED']) {
        try {
          // Presence subscription
          conn.addHandler(
              this._onReceivePresence, null, 'presence', null, null, null);
          conn.addHandler(
              this._onReceiveIQ, ChatUtil.NS['ROSTER'], 'iq', "set", null, null);
        } catch (e) {
          ChatUtil.error(e);
        }
      }
    ...
    ChatUtil.addNamespace('ROSTER_VER', 'urn:xmpp:features:rosterver');
    ChatUtil.addNamespace('NICK', 'http://jabber.org/protocol/nick');
  }
  ...
  get(Function userCallback, [String ver, List items]) {
    Map<String, String> attrs = {'xmlns': ChatUtil.NS['ROSTER']};
    this.items = [];
  ...
  subscribe(String jid, [String message, String nick]) {
    StanzaBuilder pres = ChatUtil.$pres({'to': jid, 'type': "subscribe"});
    if (message != null && message != "") {
      pres.c("status").t(message).up();
    }
    if (nick != null && nick != "") {
      pres.c('nick', {'xmlns': ChatUtil.NS['NICK']}).t(nick).up();
    }
  ...
  unsubscribe(String jid, [String message]) {
    StanzaBuilder pres = ChatUtil.$pres({'to': jid, 'type': "unsubscribe"});
  ...
  authorize(String jid, [String message]) {
    StanzaBuilder pres = ChatUtil.$pres({'to': jid, 'type': "subscribed"});
    if (message != null && message != "") pres.c("status").t(message);
    this.connection.send(pres);
  }
  ...
  unauthorize(String jid, [String message]) {
    StanzaBuilder pres = ChatUtil.$pres({'to': jid, 'type': "unsubscribed"});
    if (message != null && message != "") pres.c("status").t(message);
    this.connection.send(pres);
  }
  ...
  add(String jid, String name, [List<String> groups, Function callback]) {
    StanzaBuilder iq = ChatUtil
        .$iq({'type': 'set'}).c('query', {'xmlns': ChatUtil.NS['ROSTER']}).c(
  ...
  _onReceivePresence(xml.XmlElement presence) {
    ...
    String from = ChatUtil.getBareJidFromJid(jid);
    ...
    if (type == 'unavailable') {
      item.resources.remove(ChatUtil.getResourceFromJid(jid));
    } else if (type == null || type == '') {
      // TODO: add timestamp
      item.resources[ChatUtil.getResourceFromJid(jid)] = {
        'show': (presence.findAllElements('show').length > 0)
            ? ChatUtil.getText(presence.findAllElements('show').toList()[0])
            : "",
        'status': (presence.findAllElements('status').length > 0)
            ? ChatUtil.getText(presence.findAllElements('status').toList()[0])
            : "",
        'priority': (presence.findAllElements('priority').length > 0)
            ? ChatUtil.getText(presence.findAllElements('priority').toList()[0])
            : ""
      };
    } else {
  ...
  _onReceiveIQ(xml.XmlElement iq) {
    ...
    if (from != null &&
        ...
        from != ChatUtil.getBareJidFromJid(this.connection.jid)) return true;
    StanzaBuilder iqresult =
        ChatUtil.$iq({'type': 'result', 'id': id, 'from': this.connection.jid});
  ...
  _updateItems(xml.XmlElement iq) {
    ...
    if (queries.length != 0) {
      ...
      ChatUtil.forEachChild(query, 'item', (rosterItem) {
  ...
  _updateItem(xml.XmlElement itemTag) {
    ...
    ChatUtil.forEachChild(itemTag, 'group', (group) {
      groups.add(ChatUtil.getText(group));
    });
