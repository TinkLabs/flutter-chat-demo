...
class VCardTemp extends PluginClass {
  StanzaBuilder _buildIq(String type, String jid, [xml.XmlElement vCardEl]) {
    StanzaBuilder iq =
        ChatUtil.$iq(jid != null ? {'type': type, 'to': jid} : {'type': type});

    if (vCardEl != null) {
      iq.cnode(vCardEl);
    } else
      iq.c("vCard", {'xmlns': ChatUtil.NS['VCARD']});
    return iq;
  }

  init(ChatConnection conn) {
    this.connection = conn;
    return ChatUtil.addNamespace('VCARD', 'vcard-temp');
  }
  ...
  get(Function handlerCb, String jid, Function errorCb) {
    var iq =
        _buildIq("get", jid ?? ChatUtil.getBareJidFromJid(this.connection.jid));
  ...
  set(Function handlerCb, VCardEl vCardEl, String jid, Function errorCb) {
    ...
    StanzaBuilder iq = _buildIq("set",
        jid ?? ChatUtil.getBareJidFromJid(this.connection.jid), vCardEl.tree());
    return this.connection.sendIQ(iq.tree(), handlerCb, errorCb);
  }
}
...
class VCardEl {
  ...
  XmlElement tree() {
    StanzaBuilder build =
        ChatUtil.$build("vCard", {'xmlns': ChatUtil.NS['VCARD']})
            .c('FN')
            .t(FN)
            .up()
            .c('N')
