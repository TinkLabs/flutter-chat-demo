...
class ChatWebSocket extends ServiceType {
  ...
  ChatWebSocket(ChatConnection connection) {
...
/* * PrivateFunction: _buildStream
  ...
  StanzaBuilder _buildStream() {
    return ChatUtil.$build("open", {
      "xmlns": ChatUtil.NS['FRAMING'],
  ...
  _connectCb(bodyWrap) {
    bool error = this._checkStreamError(bodyWrap, ChatUtil.Status['CONNFAIL']);
    if (error) {
  ...
  bool _handleStreamStart(xml.XmlDocument message) {
    ...
    } else if (ns != ChatUtil.NS['FRAMING']) {
    ...
    if (error != null && error.isNotEmpty) {
      this._conn.changeConnectStatus(ChatUtil.Status['CONNFAIL'], error);
      this._conn.doDisconnect();
      return false;
    }
    return true;
  ...
  void _connectCbWrapper(message) {
    ...
    if (message.trim().indexOf("<open ") == 0 ||
      ...
      if (seeUri != null && seeUri.isNotEmpty) {
        this._conn.changeConnectStatus(ChatUtil.Status['REDIRECT'],
        ...
        this._conn.changeConnectStatus(
            ChatUtil.Status['CONNFAIL'], "Received closing stream");
        this._conn.doDisconnect();
      }
    } else {
  ...
  void _disconnect([StanzaBuilder pres]) {
    if (this.socket != null && this.socket.readyState != WebSocket.CLOSED) {
      ...
      StanzaBuilder close =
          ChatUtil.$build("close", {"xmlns": ChatUtil.NS['FRAMING']});
      this._conn.xmlOutput(close.tree());
      String closeString = ChatUtil.serialize(close.tree());
      this._conn.rawOutput(closeString);
      try {
        if (this.socket != null) this.socket.add(closeString);
      } catch (e) {
        ChatUtil.info("Couldn't send <close /> tag.");
      }
      this._conn.doDisconnect();
    }
  }
  ...
  void _onClose() {
    if (this._conn.connected && !this._conn.disconnecting) {
      ChatUtil.error("Websocket closed unexpectedly");
      this._conn.doDisconnect();
      ...
      ChatUtil.error("Websocket closed unexcectedly");
      this._conn.changeConnectStatus(ChatUtil.Status['CONNFAIL'],
          "The WebSocket connection could not be established or was disconnected.");
      this._conn.doDisconnect();
    } else {
      ChatUtil.info("Websocket closed");
    }
  }
  ...
  void _onError(Object error) {
    ChatUtil.error("Websocket error " + error.toString());
    this._conn.changeConnectStatus(ChatUtil.Status['CONNFAIL'],
        "The WebSocket connection could not be established or was disconnected.");
    this._disconnect();
  }
    ...
    if (data.length > 0 && !this._conn.paused) {
      for (int i = 0; i < data.length; i++) {
        if (data[i] != null) {
          ...
          rawStanza = ChatUtil.serialize(stanza);
  ...
  void _onMessage(dynamic msg) {
    ...
    if (this._checkStreamError(elem, ChatUtil.Status['ERROR'])) {
      return;
    }
    ...
    if (this._conn.disconnecting &&
      ...
      this._conn.rawInput(ChatUtil.serialize(elem));
  ...
  _onOpen() {
    ...
    String startString = ChatUtil.serialize(start.tree());
